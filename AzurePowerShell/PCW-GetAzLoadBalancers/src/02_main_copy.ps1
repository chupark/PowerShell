...
Import-module -Name "D:\PowerShell\PowerShell\AzurePowerShell\PCW-GetAzLoadBalancers\src\library\tools.psm1"


# Making Data Table
$col=@("num", "lbName", "lbKind", "frontEndIP", "backEndPoolName", "backEndPool", "probeRule", "ruleProto", "rulePort", "loadDistribution")
...
foreach ($lb in $lbList) {
    ...
    foreach ($lbRule in $lb.LoadBalancingRules) {
        $backendPool = ""
        $LoadDistribution = $lbRule.LoadDistribution
        ...
        $row.loadDistribution = $loadDistribution
        $table.Rows.Add($row)

        $countnum++
    }
}
#$lbList[11].LoadBalancingRules.LoadDistribution
